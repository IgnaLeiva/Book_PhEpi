...
\NormalTok{  \# Reactive expression to calculate OR or RR and custom CI}
\NormalTok{  output$result \textless{}{-} renderText(\{}
    
\NormalTok{    \# Assign values from user inputs}
\NormalTok{    A \textless{}{-} input$A\_input}
\NormalTok{    B \textless{}{-} input$B\_input}
\NormalTok{    C \textless{}{-} input$C\_input}
\NormalTok{    D \textless{}{-} input$D\_input}
\NormalTok{    CI\_level \textless{}{-} input$CI\_level / 100  \# Convert CI level to decimal}
    
\NormalTok{    \# Calculate z{-}value based on CI level}
\NormalTok{    z\_value \textless{}{-} qnorm(1 {-} (1 {-} CI\_level) / 2)}
    
\NormalTok{    \# Check whether to calculate OR or RR based on user input}
\NormalTok{    if (input$measure == "OR") \{}
\NormalTok{      \# Calculate Odds Ratio (OR)}
\NormalTok{      OR \textless{}{-} (A * D) / (B * C)}
      
\NormalTok{      \# Calculate custom CI for OR}
\NormalTok{      lower\_CI \textless{}{-} exp(log(OR) {-} z\_value * sqrt(1/A + 1/B + 1/C + 1/D))}
\NormalTok{      upper\_CI \textless{}{-} exp(log(OR) + z\_value * sqrt(1/A + 1/B + 1/C + 1/D))}
      
\NormalTok{      \# Display OR result}
\NormalTok{      paste0("Odds Ratio (OR): ", round(OR, 2), }
\NormalTok{             ", ", round(input$CI\_level, 0), "\% CI: ", round(lower\_CI, 2), " {-} ", round(upper\_CI, 2))}
      
\NormalTok{    \} else \{}
\NormalTok{      \# Calculate Risk Ratio (RR)}
\NormalTok{      risk\_exposed \textless{}{-} A / (A + B)}
\NormalTok{      risk\_unexposed \textless{}{-} C / (C + D)}
\NormalTok{      RR \textless{}{-} risk\_exposed / risk\_unexposed}
      
\NormalTok{      \# Calculate custom CI for RR}
\NormalTok{      lower\_CI \textless{}{-} exp(log(RR) {-} z\_value * sqrt((1/A) {-} (1/(A+B)) + (1/C) {-} (1/(C+D))))}
\NormalTok{      upper\_CI \textless{}{-} exp(log(RR) + z\_value * sqrt((1/A) {-} (1/(A+B)) + (1/C) {-} (1/(C+D))))}
      
\NormalTok{      \# Display RR result}
\NormalTok{      paste0("Risk Ratio (RR): ", round(RR, 2), }
\NormalTok{             ", ", round(input$CI\_level, 0), "\% CI: ", round(lower\_CI, 2), " {-} ", round(upper\_CI, 2))}
\NormalTok{    \}}
\NormalTok{  \})}
\NormalTok{\}}

\NormalTok{\# Run the application }
